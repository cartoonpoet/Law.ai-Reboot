...
const compareOldestFirst = (a: { submittedAt: string }, b: { submittedAt: string }) => a.submittedAt.localeCompare(b.submittedAt);

const ResultBadge = ({ row }: { row: InboxRow }) => {
  const variant = row.myStatus === "approved" ? css.badge.approved : row.myStatus === "rejected" ? css.badge.rejected : css.badge.neutral;
  return <span className={variant}>{row.myStatusLabel}</span>;
};

/**
 * 결재 대기함 — 내 차례가 된 결재를 처리만 하는 목록(시안 approval-process-mockup ①).
 * 통계(내 차례·예정·최근 30일 처리) → 탭(내 차례·예정·처리한 결재) → 표. 참조 받은 문서 탭·AI 브리핑은 후속.
 */
export const ApprovalInboxPage = () => {
  ...
  const items = tab === "processed" ? processed : itemsByTab[tab].toSorted(compareOldestFirst);
      ...
      <div className={css.phead}>
        <div>
          ...
          <p className={css.pdesc}>
            내 차례가 된 결재 건을 <b>처리만</b> 하는 목록입니다(여기서 결재를 새로 만들 수 없음). [처리]는 해당 문서 상세로 이동합니다. 상신은 항상 대상 문서 화면에서 합니다.
          </p>
        </div>
      </div>
      ...
      <div className={css.card}>
        <div className={css.tabs} role="tablist">
          {TABS.map((t) => (
            ...
            >
              {t.label}
              {t.value === "pending" && <span className={css.tabCount}>{pending.length}</span>}
            </button>
          ))}
        </div>

        {!isLoading && rows.length === 0 && <div className={css.empty}>{EMPTY_TEXT[tab]}</div>}
            ...
            <table className={css.table}>
              <thead>
                <tr>
                  ...
                  <th className={css.th}>{isProcessedTab ? "처리 결과" : "내 단계"}</th>
                  <th className={css.th}>상신일</th>
                  <th className={css.th}>{isProcessedTab ? "처리일" : "경과"}</th>
                  <th className={css.th} />
                </tr>
              </thead>
              <tbody>
                {rows.map((row, i) => (
                  <tr key={row.lineId} className={tab === "pending" && i === 0 ? css.rowHot : undefined}>
                    <td className={css.td}>
                      ...
                      {isProcessedTab ? (
                        ...
                        <span className={css.stepCell}>
                          <span className={css.stepPos}>
                            {row.stepNumber}
                            <small className={css.stepTotal}>/{row.totalSteps}</small>
                          </span>
                          <span className={row.isAgree ? css.badge.agree : css.badge.kind}>{row.roleLabel}</span>
                        </span>
                      )}
                    </td>
                    ...
                    <td className={css.td}>
                      {isProcessedTab ? (
                        ...
                        <span className={css.elapsed[row.elapsedTone]}>{row.elapsedLabel}</span>
                      )}
                    </td>
                    <td className={css.td}>
                      {tab === "pending" && (
                      ...
                      {tab === "upcoming" && (
                        <Button size="small" variant="outline" color="secondary" onClick={() => navigate(row.href)}>
                          보기
                        </Button>
                      )}
                    </td>
