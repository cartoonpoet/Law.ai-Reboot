...
import { COUNTRY_OPTIONS, REGION_OPTIONS } from "../adviceMeta";
...
export const AdviceDetailSection = () => {
  const {
    control,
    formState: { errors },
    ...
    <Card bordered header={<CardTitle num={2}>상세정보</CardTitle>}>
      <div className={css.detailStack}>
        ...
        <Field label="사안의 배경" required>
          ...
          <div className={formCss.fieldBlock}>
            <Controller
              name="background"
              control={control}
              render={({ field }) => (
        ...
        <Field label="기타 요청사항">
          <Controller
            ...
            control={control}
            render={({ field }) => (
              <Textarea rows={3} placeholder="회신 형식 · 참고 사항 등 그 밖에 알려줄 내용" value={field.value} onChange={field.onChange} />
            )}
          />
        </Field>
