...
import { checkStreamConnected } from "./notificationStreamState";

// 알림 쿼리 키. 다른 도메인(useComments 등)에서 invalidate 시 재사용하도록 export.
export const NOTIFICATIONS_QUERY_KEY = ["notifications"] as const;
const NOTIFICATIONS_LIMIT = 20;
// SSE(useNotificationStream)가 주 경로이므로 연결돼 있으면 폴링은 5분 백업.
const POLL_INTERVAL_MS = 300_000;
// 실시간 연결이 끊겼을 때만 촘촘히 확인한다(늦어도 30초 안에는 새 알림이 보인다).
...
export const useNotifications = () => {
  ...
  const query = useQuery({
    ...
    refetchInterval: () => (checkStreamConnected() ? POLL_INTERVAL_MS : OFFLINE_POLL_INTERVAL_MS),
